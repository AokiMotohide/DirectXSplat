...
Status Application::Load(const std::filesystem::path& scenePath) {
  ...
  DestroyUploadedScenes();
  sceneManager_.Clear();
  Status uploadStatus = UploadAndAddScene(std::move(sceneResult.value));
  if (!uploadStatus.ok) {
    return uploadStatus;
  }
  if (sceneManager_.ActiveScene() != nullptr && !sceneManager_.ActiveScene()->inputCameras.empty()) {
    selectedInputCamera_ =
        std::clamp(selectedInputCamera_, 0, static_cast<int32_t>(sceneManager_.ActiveScene()->inputCameras.size() - 1));
  } else {
