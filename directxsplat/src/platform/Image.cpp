...
directxsplat::Status ComputePpmBytes(uint32_t width, uint32_t height, size_t& outBytes) {
  outBytes = 0;
  if (width == 0 || height == 0 || width > kMaxPpmDimension || height > kMaxPpmDimension) {
    return directxsplat::Status::Error("invalid ppm dimensions");
  }
  ...
  if (pixels > std::numeric_limits<size_t>::max() / 4u || pixels > kMaxPpmBytes / 4u) {
    return directxsplat::Status::Error("ppm image is too large");
  }
...
directxsplat::Status SavePpm(const ImageRgba8& image, const std::string& path) {
  size_t imageBytes = 0;
  const directxsplat::Status sizeStatus = ComputePpmBytes(image.width, image.height, imageBytes);
  if (!sizeStatus.ok || image.Empty() || image.pixels.size() < imageBytes) {
    return directxsplat::Status::Error("invalid image");
  }

  std::ofstream file(path, std::ios::binary);
  if (!file.is_open()) {
    return directxsplat::Status::Error("failed to open output");
  }
  ...
  if (!file) {
    return directxsplat::Status::Error("failed to write output");
  }
  return directxsplat::Status::Ok();
}
...
directxsplat::StatusOr<ImageRgba8> LoadPpm(const std::string& path) {
  ...
  if (!file.is_open()) {
    return directxsplat::StatusOr<ImageRgba8>::Error("failed to open image");
  }

  std::string magic;
  file >> magic;
  if (magic != "P6") {
    return directxsplat::StatusOr<ImageRgba8>::Error("unsupported ppm magic");
  }
  ...
  if (!file) {
    return directxsplat::StatusOr<ImageRgba8>::Error("invalid ppm header");
  }
  ...
  size_t imageBytes = 0;
  const directxsplat::Status sizeStatus = ComputePpmBytes(width, height, imageBytes);
  if (!sizeStatus.ok) {
    return directxsplat::StatusOr<ImageRgba8>::Error(sizeStatus.message);
  }
  ...
  try {
    ...
    return directxsplat::StatusOr<ImageRgba8>::Error("ppm allocation failed");
  } catch (const std::length_error&) {
    return directxsplat::StatusOr<ImageRgba8>::Error("ppm image is too large");
  }
  ...
  for (size_t i = 0; i < pixelCount; ++i) {
    ...
    if (r == std::char_traits<char>::eof() || g == std::char_traits<char>::eof() || b == std::char_traits<char>::eof()) {
      return directxsplat::StatusOr<ImageRgba8>::Error("truncated ppm data");
    }
  ...
  return directxsplat::StatusOr<ImageRgba8>::Ok(std::move(image));
}
